...
simulation_t = function(param, n){
  ...
  for(i in 1:n){
    x = sample(0:1, 1)
    pi_01 = rbinom(1, 1, 1-exp(param[3]))
    unif1 = runif(1, min = 0, max = 1)
    S01 = function(T)
    {
      1-pweibull(T, exp(param[1] + param[8]*x), exp(param[2])) - unif1
    }
    T_01[i] = uniroot(S01, interval = c(1.e-14, 1e04), tol = 1e-9)$root
    unif2 = runif(1, min = 0, max = 1)
    S12 = function(T)
    {
      1-pweibull(T, exp(param[4]+ param[9]*x), exp(param[5])) - unif2
    }
    T_12[i] = uniroot(S12, interval = c(1.e-14, 1e04), tol = 1e-9)$root
    unif3 = runif(1, min = 0, max = 1)
    S02 = function(T)
    {
      1-pweibull(T, exp(param[6] + param[10]*x), exp(param[7])) - unif3
    }
    ...
    t_n = length(t)
    if((t[t_n] < T_01[i] & pi_01 == 0) | (t[t_n] < T_02[i] & pi_01 == 1)){
...
sum_all_comp = function(a, t_type0, t_type1, t_type2, t_type3, mc_cores){
  type_1_comp = -sum(unlist(mclapply(1:nrow(t_type0), function(i) (log(S_01_comp(a = a,  t = t_type0[i, 1], x = t_type0[i, 2])*S_02_comp(a = a,  t = t_type0[i, 1],
                                                                                                                                         x = t_type0[i, 2]))))))
  ...
  type_4_comp = -sum(unlist(mclapply(1:nrow(t_type3), function(i) (log(f_02_comp(a = a,  t = t_type3[i, 1], x = t_type3[i, 2])*S_01_comp(a = a,  t = t_type3[i, 1], x = t_type3[i, 2]))))))
  
  return(type_1_comp + type_2_comp + type_3_comp + type_4_comp) 
}

sum_all_embed = function(a, t_type0, t_type1, t_type2, t_type3, mc_cores){
  type_1_embed = -sum(unlist(mclapply(1:nrow(t_type0), function(i) (log(S_01_embed(a = a,  t = t_type0[i, 1], 
                                                                                   x = t_type0[i, 2])+S_02_embed(a = a,  t = t_type0[i, 1], x = t_type0[i, 2]))))))
...
for(mn in 1:nn){
  ...
  param_embed = c(0.4,  0.6, -0.7, 1.6, -0.5, 0.2,  1.1, -0.9, -1.2, 0)
  n = 1000
  t = simulation_t(param_embed, n)
  ...
  theta_hat_comp = nlminb(param_comp, sum_all_comp,t_type0 = t_type0, t_type1 = t_type1, t_type2 = t_type2, t_type3 = t_type3, mc_cores = 5)
  
  theta_hat_embed = nlminb(param_embed, sum_all_embed,t_type0 = t_type0, t_type1 = t_type1, t_type2 = t_type2,
                           t_type3 = t_type3, mc_cores = 5)
  ...
  theta_hat_hessian_embed = hessian(sum_all_embed, theta_hat_embed$par, t_type0 = t_type0, t_type1 = t_type1, t_type2 = t_type2, t_type3 = t_type3, mc_cores = 5)
