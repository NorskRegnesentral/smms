...
## Input: gg (the graph)
names_of_survival_density = function(gg){
...
survival_function = function(gg, type, possible_passedBy){
  ...
  if(length(all_colnames) > 0){
    ...
    for(i in 1:length(all_colnames)){
      which_transition[i] = which(names_of_survival_density(gg)[, "all_transitions"] == all_colnames[i])
      survival_function[i] = names_of_survival_density(gg)[which_transition[i], "survival_name"]
    }
  }
  else
...
density_function = function(gg, type, travelled){
  ...
  if(length(all_colnames) > 0){
    ...
    for(i in 1:length(all_colnames)){
      which_transition[i] = which(names_of_survival_density(gg)[, "all_transitions"] == all_colnames[i])
      density_function[i] = names_of_survival_density(gg)[which_transition[i], "density_name"]
    }
  }
  else
...
type_to_integrand = function(type, gg){
  ...
  all_absorbing = which(!(all_edges[,2] %in% all_edges[,1]))
  absorbing_state = unique(sapply(all_absorbing, function(p) all_edges[p,2]))
  which_in_absorbing = which(names_of_survival_density(gg)[,"to_prev"] == absorbing_state)
  types_to_absorbing = sapply(which_in_absorbing, function(p) names_of_survival_density(gg)[p,"all_transitions"]) 
